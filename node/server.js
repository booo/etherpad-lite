...
async.waterfall([
    ...
    function (settings, callback) {
        DB.init(settings, function(error, db){
            if(error) {
                callback(error);
            } else {
                callback(null, settings, db);
            }
        });
    },
    //initalize the http server
    function (settings, db, callback) {
        //create server
        ...
        app.readOnlyManager = new ReadOnlyManager(db);

        app.authorManager = new AuthorManager(db);

        app.padManager = new PadManager(settings, db, app.authorManager, app.readOnlyManager);

        app.groupManager = new GroupManager(db, app.padManager, null);

        app.sessionManager = new SessionManager(db, app.groupManager, app.authorManager);

        app.securityManager = new SecurityManager(settings, db, app.authorManager, app.padManager, app.sessionManager);
        ...
        app.error(function(err, req, res, next){
            //TODO add global error prototype
            res.send(500);
